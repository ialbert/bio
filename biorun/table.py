...
@plac.flg("features", "extract features only ", abbrev='G')
...
def run(start='1', end='', type_='', id_='', match='', gene='',
        rename='', features=False, olap='', fields="id,size",  *fnames):
    """
    ...
    convert.run(protein=False, translate=False, start=start, table=True, fields=fields,
                end=end, type_=type_, id_=id_, revcomp=False, features=features, olap=olap,
